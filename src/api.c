...
Datum ttl_create_index(PG_FUNCTION_ARGS)
{
    ...
    appendStringInfo(&query,
                     "INSERT INTO ttl_index_table (table_name, column_name, "
                     "expire_after_seconds, active, created_at) "
                     ...
                     quote_literal_cstr(column_name), expire_after_sec);

    ret = SPI_exec(query.data, 0);
    success =
        (ret == SPI_OK_INSERT || ret == SPI_OK_UPDATE || ret == SPI_OK_UTILITY);
...
Datum ttl_drop_index(PG_FUNCTION_ARGS)
{
    text *table_name_text = PG_GETARG_TEXT_PP(0);
    text *column_name_text = PG_GETARG_TEXT_PP(1);
    char *table_name = text_to_cstring(table_name_text);
    ...
    initStringInfo(&query);
    appendStringInfo(&query,
                     "DELETE FROM ttl_index_table WHERE table_name = %s AND "
                     "column_name = %s",
                     quote_literal_cstr(table_name),
